(function(){
    ...
    var agendado = {
        controller: agendadoCtrl,
        templateUrl: 'app/components/agendado/agendado.html'
    }

    angular
        .module("jelpMi")
        .component("servicioJelp", agendado);

    agendadoCtrl.$inject = ["$location", "$firebaseArray", "$firebaseObject", "authfire","$http", "$interval"];
    function agendadoCtrl( $location, $firebaseArray,$firebaseObject, authfire,$http, $interval){
      ...
      ref.orderByKey().equalTo(agen.user.uid).on('value',
        function(data){
          ...
          if (direcciones[k].direcciones) {
            ...
            console.log("ahahjshjsahj",direcciones[k].direcciones.length)
            var testCode = agen.direccionesArray;
              var name;
              var e = 1;
              for (name in testCode){
              testCode[name].code = name;
              testCode[name].number = e;
              e++;
              if(testCode[name].number == 1){
                agen.direccionElegida = testCode[name];
                agen.codeDireccion = testCode[name].code;
                agen.flagDireccionLocked = true;
                console.log(agen.flagDireccionLocked)
              }
            }
            console.log()
          }else {
          ...
          agen.checkDireccion = function () {
            console.log("XD",agen.direccionesArray)
          }
          ...
          agen.incremento = function(item){
            ...
            if(item.count <6){
              ...
              item.count +=1;
              item.partial += item.price;
              agen.real_total += item.price;
              //agen.sumaParcial()
            }
          }
          ...
          agen.promociones = function(){
            ...
            var promos = userData[k].promociones;
            var keyP;
            for(keyP in promos){
              if(agen.promoCode == promos[keyP].code){
                Materialize.toast('Ya has utilizado este código', 6000, 'purple darken-3');
                agen.flagPromoUsada = true;
                break;
              }

            }
            if(!agen.flagPromoUsada){
              agen.promosRef.$loaded().then(
                function(x){
                  var d
                  for(d in x){
                    if (x[d].code == agen.promoCode) {
                      agen.flagPromoValida = true;
                      agen.descPromo = x[d].descripcion;
                  ...
                  console.log("ahahahahah", ajaja)
                if(!agen.flagPromoValida){
                  Materialize.toast('Código inválido', 4000, 'purple darken-3')
                }else{
                  Materialize.toast('Código Válido, Ahora tienes: '+ agen.descPromo, 6000, 'purple darken-3');
                  agen.promo = false;
                  agen.flagPromoValida = false;
                }
              })
            }
          ...
          agen.enviarServicio = function(){
            agen.periodClave = $('#agendado-periodo').val()
            agen.inicio = $('#agendado-inicio').pickadate().pickadate('picker').get()
            if(agen.periodClave == 1){
              agen.periodo = "Semanal"
            } else if(agen.periodClave == 2){
              agen.periodo = "Quincenal"
            } else if(agen.periodClave == 3){
              agen.periodo = "Mensual"
            } else if(agen.periodClave == 4){
              agen.periodo = "Cada 21 Días"
            }
            ...
            if(agen.periodo && agen.hora && agen.inicio){
              ...
              alert("debes proporcionar fecha de inicio, Horario y periodicidad")
            }
    ...
    //STRIPE
        ...
        //Crear tarjeta
          ...
          agen.boton = document.getElementById("btnAdicionales");
          console.info("boton", agen.boton)

          agen.parpadeo = function() {

            if (!agen.a){
              agen.boton.style.transform="scale(1.01)"
              agen.a = true;
            } else{
              agen.boton.style.transform="scale(1)";
              agen.a = false;
            }

          };

          agen.setInterval = function () {
            agen.boton = document.getElementById("btnAdicionales");
            agen.interval = $interval(agen.parpadeo, 500);
          }




          agen.testing = function() {
            console.log("ja")
            $interval.cancel(agen.interval);
